...
- フェイント表示中(`Phase::Feint`)は、`FEINT_TEXT`(「撃つな」)の見出しを、本物より紛らわしい色(`FEINT_BG`、合図の緑よりくすんだ黄緑〜山吹色)の背景で出す。画像(`SIGNAL_PNG`、「撃て!」の画像)は本物の合図専用のままとし、フェイントには使わない(常にテキスト表示)
...
- フェイント表示中の見出しが「撃つな」であり、本物の合図の文字(`SIGNAL_TEXT`、「撃て!」)を含まないこと
